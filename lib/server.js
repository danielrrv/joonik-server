...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        if (state !== stateCookie) {
            return res.status(403).send(`Request origin cannot be verified previous:${state} now:${'Nose'}`);
        }
...
const redirection = function (req, res, next) {
    if (!cookie.parse(req.headers.cookie.state) || !cookie.parse(req.headers.cookie._vl)) {
...
app.use((err, req, res, next) => {
    ...
    res.status(status);
    res.send('Error Fatal del servidor.');
});
